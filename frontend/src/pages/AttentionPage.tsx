...
export function AttentionPage() {
  ...
  return <section className="page attention-page">
    ...
    {loading ? <p className="loading-copy">Loading attention queue…</p> : items.length === 0 ? <div className="empty-state attention-empty"><span className="empty-mark">✓</span><p>No tasks need attention right now.</p></div> : <div className="attention-list">{items.map((item) => <Link className="attention-row" key={item.id} to={`/projects/${item.project_id}`}><div className="attention-project">{item.project_name}</div><div className="attention-task"><h2>{item.title}</h2><span>{STATUS_LABELS[item.status]}{item.agent ? ` · ${item.agent}` : ""}</span></div><div className="attention-tags">{item.tags.filter((tag) => attentionTags.has(tag.name)).map((tag) => <TagBadge key={tag.id} tag={tag} />)}</div><span className="tile-arrow">→</span></Link>)}</div>}
  </section>;
}
