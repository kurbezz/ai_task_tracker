...
/** Marks a matching pending intent only after its get_task call succeeded. */
export function markPendingAttachmentConfirmable(state: SessionState, taskId: string): boolean {
  if (state.pendingAttachmentTaskId === taskId) {
    state.pendingAttachmentConfirmableTaskId = taskId;
    return true;
  }
...
/** Public surface returned by {@link createTurnTracker}. */
export interface TurnTracker {
   ...
   */
  confirmPendingAttachment(sessionID: string): Promise<boolean>;
  queueValidatedAttachment(sessionID: string, taskId: string): Promise<boolean>;
 ...
 */
export function createTurnTracker(store: SessionStore, dependencies: TurnTrackerDependencies): TurnTracker {
  const now = dependencies.now ?? (() => new Date());
  const author = requireAuthor(dependencies.author ?? "omp");
  ...
  return {
    ...
    onCommandExecuteBefore(input: CommandExecuteBeforeInput): void {
      ...
      if (!state.pendingAttachmentTaskId) {
        state.pendingAttachmentTaskId = taskId;
        state.pendingAttachmentConfirmableTaskId = null;
      }
    },
    ...
    async queueValidatedAttachment(sessionID: string, taskId: string): Promise<boolean> {
      const state = store.get(sessionID);
      if (!state.validatedTaskIds.delete(taskId)) return false;

      const attachment: PendingAttachment = {
        event: {
          eventId: `task-attached:${randomUUID()}`,
          taskId,
      ...
      try {
        const queued = await persistOnce(
          sessionID,
          attachment,
          dependencies.queue,
          inFlight,
        ...
        if (!queued) state.validatedTaskIds.add(taskId);
        return queued;
      } catch (error) {
        state.validatedTaskIds.add(taskId);
        throw error;
      }
    },
...
async function persistAttachment(
  ...
  const event: TrackingEvent = {
    ...attachment.event,
    idempotencyKey: `omp:${sessionID}:${attachment.event.eventId}`,
    sessionId: sessionID,
    kind: "task-attached",
    author,
...
async function persistProgress(
  ...
  const event: TrackingEvent = {
    ...progress.event,
    idempotencyKey: `omp:${sessionID}:${progress.event.eventId}`,
    sessionId: sessionID,
